...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    this.loadSessionAndUpdate(this.router.url);
    ...
    this.router.events
      ...
      .subscribe((event) => {
        this.loadSessionAndUpdate(event.urlAfterRedirects);
      });
  }

  private loadSessionAndUpdate(url: string) {
    this.http.get<any>('http://localhost:8000/api/session', {
      withCredentials: true
    }).subscribe({
      next: (res) => {
        this.userRole = res.role ?? null;
        this.updateChatVisibility(url);
      },
      error: () => {
        this.userRole = null;
        this.updateChatVisibility(url);
